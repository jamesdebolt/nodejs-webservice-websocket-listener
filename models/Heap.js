...
class Heap {
  ...
  // remove and return the top of the heapF
  pop() {
  ...
  bubbleDown(index) {
    while (true) {
      ...
      if (
        childLeftIndex < this.heap.length && // left must be a valid index
        this.sortFunction(this.heap[childLeftIndex], this.heap[index]) < 0 && // left must be a higher prio
        (childRightIndex >= this.heap.length ||
          this.sortFunction(
            this.heap[childLeftIndex],
            this.heap[childRightIndex]
          ) <= 0) // left must be bigger than/equal to right or right doesnt exist
      ) {
        shouldSwapLeft = true;
      } else if (
        childRightIndex < this.heap.length && // right must be a valid index
        this.sortFunction(this.heap[childRightIndex], this.heap[index]) < 0 && // right must be a higher prio
        (childLeftIndex >= this.heap.length ||
          this.sortFunction(
            this.heap[childLeftIndex],
            this.heap[childRightIndex]
          ) > 0) // right must be bigger than left or left doesnt exist
      ) {
        shouldSwapRight = true;
      }

      if (shouldSwapLeft) {
        [this.heap[index], this.heap[childLeftIndex]] = [
          this.heap[childLeftIndex],
          this.heap[index],
        ];
      ...
      if (shouldSwapRight) {
        [this.heap[index], this.heap[childRightIndex]] = [
          this.heap[childRightIndex],
          this.heap[index],
        ];
